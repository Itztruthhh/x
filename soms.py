...
# Apply nest_asyncio to fix "RuntimeError: This event loop is already running"
nest_asyncio.apply()
...
async def process_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args  # Extract command arguments correctly
    if len(args) != 2:
        await update.message.reply_text("Usage: /buy <server> <duration>")
        return
...
async def main():
    ...
    app.add_handler(CommandHandler("buy", show_servers))  # Fallback for /buy command
